// --------------------------------------------------------------------------------------------------------------------
// <著作権 ファイル="OnJoinedInstantiate.cs" 会社="Exit Games GmbH">
//   部分：Photon Unity Utilities,
// </著作権>
// <概要>
//   このコンポーネントは、ルームに参加したときにネットワーク用のGameObjectをインスタンス化します
// </概要>
// <著者>developer@exitgames.com</著者>
// --------------------------------------------------------------------------------------------------------------------
...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class OnJoinedInstantiate : MonoBehaviour, IMatchmakingCallbacks {
        ...
        // 「スポーン」とは、ゲームの中でキャラクターやオブジェクトが生成されることを意味するんだ。だから、「スポーンシーケンス」は、キャラクターやオブジェクトが特定の順序や条件に従って生成される一連のプロセスを指す言葉
        public enum SpawnSequence { Connection, Random, RoundRobin }

        #region Inspector Items

        // 「古いフィールド、後方互換のためにここにのみ存在します。OnValidateでSpawnPointsに値がコピーされます」
        ...
        [Tooltip("「スポーンポイントの位置にランダムな変動を加えます。GetRandomOffset()は、オフセットを生成するための独自の方法で上書きすることができます」")]
        public bool UseRandomOffset = true;

        [Tooltip("ランダムオフセットの半径")]
        [UnityEngine.Serialization.FormerlySerializedAs("位置オフセット")]
        public float RandomOffset = 2.0f;

        [Tooltip("ランダムオフセットのY軸を無効にします。スポーンポイントのY値が使用されます。")]
        public bool ClampY = true;

        public List<GameObject> PrefabsToInstantiate = new List<GameObject>(1) { null }; // set in inspector

        [UnityEngine.Serialization.FormerlySerializedAs("自動でオブジェクトを生成する（スポーンする）")]
        public bool AutoSpawnObjects = true;

        #endregion

        // スポーンされたオブジェクトの記録、すべてをデスポーンするために使用されます
        ...
        {
            /// プレハブをチェックして、それが実際のリソースであり、シーンオブジェクトや他のインスタンスではないことを確認します。
            ...
            /// 古いSpawnPositionフィールドから新しいSpawnPointsへと、どんな値でも移動させます
            if (SpawnPosition)
            {
                if (SpawnPoints == null)
        ...
        /// <summary>
        /// 検証し、有効であればこのプレハブをリストの最初のnull要素に追加するか、新しい要素を作成します。オブジェクトが追加された場合はtrueを返します。
        /// </summary>
...
//        void Add(SerializedProperty list, int i)
//        {
//            {
//                int newindex = list.arraySize;
